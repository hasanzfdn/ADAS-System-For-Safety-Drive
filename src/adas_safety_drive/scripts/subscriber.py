...
def process_image(image):
    try:
        ...
        waypoints = [np.mean(leftx) + 300, np.mean(lefty) - 300]
...
def imageInfoCallback(data,waypoint_pub):
    ...
    waypoints= process_image(cv_image)

    wp = wp_message()
    wp.waypoint_x = waypoints[0]
    wp.waypoint_y = waypoints[1]
...
def cameraInfoListener():
    ...
    rospy.wait_for_message('simulator/middle_camera/image/compressed',CompressedImage)
    ...
    rospy.Subscriber('simulator/middle_camera/image/compressed', CompressedImage, imageInfoCallback, waypoint_publisher)
